...
const bucket = '"movimientos"';
//const dataquest = 'from(bucket: ' + bucket + ') |> range(start: 2022-06-17T00:00:00Z, stop: 2022-06-17T10:00:00Z)'; //FLUX Query just data range 
const dataquest = 'from(bucket: ' + bucket + ') |> range(start: 2022-06-17T00:00:00Z, stop: 2022-06-17T10:00:00Z) |> aggregateWindow(every: 5m, fn: mean, createEmpty: false)'; //FLUX Query data range with data "Downsampling" for high Volume data
