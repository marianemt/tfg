...
def cr1(fich, numRandom):
    header = ['idMuestra', 'Milimetros', 'SensorMedia', 'Time']
    datos = []
    ...
    for i in range(1, numRandom + 1):
        ...
        muestra = (i, np.random.randint(low=0, high=1001), '<ID_sensor_' + str(np.random.randint(low=1, high=21)) + '>', datetime.now())
